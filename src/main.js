...
const presenters=createPresenters(scene,camera,environment.uniforms,ripples);
...
function render(delta=0,diagnostics){
  ...
  act.underwater=wetness;
  presenters.update(act,time);
...
function frame(now){if(paused!==motion.matches){paused=motion.matches;if(paused)ripples.reset();}previous=previous||now;const raw=(now-previous)/1000;const dt=Math.min(raw,.05);previous=now;if(!paused&&!manual){journey.tick(now);time+=dt;ripples.update(dt);perf.frame(raw);}render(dt);if(!paused&&!manual)raf=requestAnimationFrame(frame);}
...
addEventListener('pagehide',event=>{cancelAnimationFrame(raf);if(event.persisted)return;loader.dispose();sceneControls.dispose();logo.dispose();waveType.dispose();journey.dispose();post.dispose();particles.dispose();ripples.dispose();ocean.dispose();environment.dispose();renderer.dispose();});
...
function projectPointer(event){
  ...
  const fromBelow=act.underwater>.5;
  if(fromBelow?ray.direction.y<=.015:ray.direction.y>=0)return null;
  let prior=.1;
  for(let t=.6;t<130;t+=.5){
    ray.at(t,hit);
    const surface=heightAtWorld(hit.x+flow.x,hit.z+flow.y,time,act);
    if(fromBelow?hit.y>=surface:hit.y<=surface){
      let low=prior,high=t;
      for(let i=0;i<9;i++){
        const middle=(low+high)*.5;ray.at(middle,hit);
        const h=heightAtWorld(hit.x+flow.x,hit.z+flow.y,time,act);
        if(fromBelow?hit.y<h:hit.y>h)low=middle;else high=middle;
      }
...
function interact(event){
  if(paused||document.hidden||(act.underwater>.5&&act.progress<.9))return;
  ...
  const strength=act.progress>=.9?.035:(isTap?.22:.085);
  if(ripples.inject(point.x,point.z,strength)){
...
window.__oceanStudy={
  ...
  advanceArrival(steps=1){for(let i=0;i<steps;i++){time+=1/60;ripples.update(1/60);render(1/60);}return {act,ripples:ripples.stats};},

  presenterStats(){return presenters.stats();},
...
document.querySelector('#scroll-cue').addEventListener('click',()=>{journey.finishIntro();window.scrollBy({top:innerHeight*.25,behavior:'instant'});});
resume();
